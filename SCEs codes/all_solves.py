...
The user does not need to run this code, but this how the sces are solved
"""
...
sys.path.insert(0, file_directory_name.removesuffix("\\sces codes") + \
...
def solve_sces(parameters : Union[list, dict],
    ...
    Solve self-consistency equations for multiple parameter sets
    

    Parameters
    ----------
    parameters : Union[list, dict]
        List, or single set of parameters
    solved_quantities : list[str]
        Quantities to solve (e.g., phi_N).
    bounds : Union[list[tuple[float], tuple[float]], list[list[tuple[float], tuple[float]]]]
        List of, or set of bounds for the solved_quantities.
        Each set of bounds is a list of 2 tuples. The first tuple contains the lower
        bound for each solved quantitiy, the second contains the upper bound
    x_init : Union[npt.NDArray, list[npt.NDArray]]
        List of, or set of initial values for the solved_quantities.
    solver_name : Literal['basin-hopping', 'least-squares']
        Solver used to solve the self-consistency equations.
        'basin-hopping' = global solve, 'least-squares' = local solve.
    solver_kwargs : Union[dict, list], optional
        List of, or single dict containing arguments for the local solver.
        The default is {'xtol' : 1e-13, 'ftol' : 1e-13}.
    other_kwargs : Union[dict, list], optional
        List of, or single dict containing arguments for the global solver, if 
        used. The default is {}.
    include_multistability : bool, optional
        Should the solver solve for the stbaility boundary. The default is False.

    Returns
    -------
    sol : pd.DataFrame
        Dataframe where each row consistists of 
            the parameters set
            the solved self-consistency equations
            the loss function (to see whether the sces were solved well)
            stability-related quantities (<(dNde)^2> and <(dRde)^2>)
            other important properties
                growth-consumption correlation
                species packing ratio
                infeasibiltiy distance (phi_R - phi_N * gamma^{-1}) (=0 when 
                 dynamics are infeasibile)
    '''
    
    # add scaler for solved quantities so that tbeir contributions to the loss
    # function are of similar orders of magnitude (improves solve)
    ...
    # call function to solve self consitency equations for different
    # parameter sets
    ...
    # calcualte important properties 
    sol['rho'] = np.sqrt(1 / (1 + \
...
def Global_Solve_SCEs(varying_parameter : str,
    ...
    Globally solve sets of self-consistency equations 

    Parameters
    ----------
    varying_parameter : str
        The name of the parameter being varied alongside the resource pool size.
    p_range : tuple[float, float]
        Min. and max. values of the varying parameter
    n : int
        Number of parameter values to sample withint the p_range
    filename : str
        File directory to save self-consistency equations.
    resource_pool_sizes : npt.NDArray, optional
        Resource pool sizes to solve for. The default is np.arange(50, 275, 25).
    any_fixed_parameters : dict, optional
        Any parameters that may be fixed + their values (also includes 
        varying_parameter, which will be excluded later).
        The default is dict(mu_c = 160, sigma_c = 1.6,
                            mu_y = 1, sigma_y = 0.130639,
                            mu_d = 1, sigma_d =  0,
    ...
    def clean_bad_solves(sces, solved_quantities, bounds, x_init,
        ...
        Repeat solving protocol for self-consistency equations that have not 
        been solved well (large loss function).
        Runs the global solve for more iterations 
    ...
    directory = file_directory_name.removesuffix("\\sces codes") + \
                    "\\Data\\self_consistency_equations"
    ...
    # remove the parameter being varied from the fixed parameter dict
...
def Solve_Stability_Boundary(solved_sces : any,
    ...
    Locally solve the stability boundary across values of M.
    
    The initial conditions are the set of parameters + self-consistency equations 
    closest to the stability boundary for each value of M.
    This data is interpolated, then solved.
    

    Parameters
    ----------
    solved_sces : any (pd.DataFrame)
        The solved self-consistency equations + parameters (sol).
    varying_parameter : str
        The parameter being varied alongside M.
    interpolation_protocol : Literal['A', 'B']
        Different data interpolation protocols have been fine tuned to different parameters.
        The argument tells you which protocol to use for which parameter
    parameter_bounds : Union[None, list[float, float]] 
        bounds on the parameter being varied 

    Returns
    -------
    solved_phase : pd.DataFrame
        Dataframe where each row consistists of 
            the parameters set
            the solved self-consistency equations
            the loss function (to see whether the sces were solved well)
    ...
    directory = file_directory_name.removesuffix("\\sces codes") + \
                    "\\Data\\self_consistency_equations\\stability_bound"
...
sces_mu_c = pd.read_pickle(file_directory_name.removesuffix("\\SCEs codes") + \
                           "\\Data\\self_consistency_equations" + \
                               "M_vs_mu_c.pkl")
...
sces_sigma_c = pd.read_pickle(file_directory_name.removesuffix("\\SCEs codes") + \
                              "\\Data\\self_consistency_equations" + \
                                  "M_vs_sigma_c.pkl")
...
sces_sigma_y = pd.read_pickle(file_directory_name.removesuffix("\\SCEs codes") + \
                              "\\Data\\self_consistency_equations" + \
                                  "M_sigma_y.pkl") 
