...
def CRM_vs_egLV_plot(CRM_simulation,
                     egLV_simulation,
                     filename_save,
                     mu_c = 145):
    ...
    # Extract M and max. LEs for the CRM with relevent mu_c
    ...
    # construct standard error bars for probabilities/proportions
...
def eLV_interact_stats_plot(competition_df, filename_save,
    ...
    Plot M vs statisitcal properties of inter-species interactions and 
    self-inhibition.
...
def read_eLV_data(subdirectory):
    ...
    Read in eLV communities and extract M vs max. LE
...
CRM_df = generate_simulation_df(file_directory_name.removesuffix("\\Figures") + \
