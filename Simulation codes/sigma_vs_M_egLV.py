...
sys.path.insert(0, file_directory_name.removesuffix("\\simulation codes") + \
                "\\Modules\\Consumer-resource models")
...
def egLV_sigma(sigma_name : Literal['sigma_c', 'sigma_y'],
    ...
    CRM_directory : str
        File directory to extract CRMs from (for parametrising the eLVs)
    M : float, optional
        The default is 225.
    ...
    def read_call_egLV(filename : str, cavity_phi_R : float):
        ...
        CRM_communities = pd.read_pickle(file_directory_name.removesuffix("\\simulation codes") + \
        ...
        # save eLV
        pickle_dump(file_directory_name.removesuffix("\\simulation codes") + \
                    "\\Data\\simulation_data\\egLV\\" + \
    ...
    if not os.path.exists(file_directory_name.removesuffix("\\simulation codes") + \
                          "\\Data\\simulation_data\\egLV\\"  + \
                          gLV_directory):
        
        os.makedirs(file_directory_name.removesuffix("\\simulation codes") + \
                    "\\Data\\simulation_data\\egLV\\" + \
                    gLV_directory)
       
    # load in self-consistency equation data
    sces = pd.read_pickle(file_directory_name.removesuffix("\\simulation codes") + \
                          "\\Data\\self_consistency_equations\\"+ \
                          + CRM_directory + ".pkl")
    
    # extract values of sigma from sces, used for extracting and naming files
    # (inefficient) 
