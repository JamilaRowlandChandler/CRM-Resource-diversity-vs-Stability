...
def M_effect_fixed_C_fixed_supply(M_range, mu_C_range, sigma_C, b, n,
                                  fixed_parameters):
    
    parameters = generate_parameters(M_range, mu_C_range, sigma_C, b, n,
                                     fixed_parameters)
    
    CRM_across_parameter_space(parameters,
                               file_directory_name.removesuffix("\\simulation codes") + \
                                   "\\Data\\simulation_data\\M_vs_mu_c_fixed_supply",
                               ['M', 'mu_c'])
                    
# %%

def generate_parameters(M_range, mu_C_range, sigma_C, b, n,
                        fixed_parameters):
    ...
    variable_parameters = np.vstack([M_mu_C_combinations[0, :]/fixed_parameters['gamma'],
                                     ...
                                     np.repeat(b, M_mu_C_combinations.shape[1])/M_mu_C_combinations[0, :]])
...
M_effect_fixed_C_fixed_supply(resource_pool_sizes, np.array([100, 250]), 1.6, 150,
                              11, {'mu_y': 1, 'sigma_y' : 1.6/np.sqrt(150),
                                   'd' : 1, 'gamma' : 1})
